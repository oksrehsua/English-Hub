    ...
    if (isDictationMode) {
        ...
        if (match) {
            ...
            options.forEach((opt, idx) => {
                ...
                label.innerHTML = `<input type="radio" name="answer" value="${opt}"> <span style="font-weight:700; opacity:0.5; margin-right:4px; font-size:0.9em;">${idx + 1}.</span> ${opt}`;
...
document.addEventListener('keydown', function (event) {
    ...
    // 数字キー (1〜9) で選択問題の選択肢をショートカット選択
    if (event.key >= '1' && event.key <= '9') {
        // テキスト入力中は無効にする
        const activeTagName = document.activeElement.tagName.toLowerCase();
        if (activeTagName === 'input' && document.activeElement.type === 'text') return;
        if (activeTagName === 'textarea') return;

        const radios = document.querySelectorAll('input[name="answer"][type="radio"]');
        if (radios.length > 0) {
